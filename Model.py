...
# Filling NaN values in the BMI feature using the median:
...
# Renaming some names in the worktype feature for simplicity:
dataset.replace({'Self-employed' : 'Self_employed'}, inplace=True)

# Renaming some names in the smoking status feature for simplicity:
...
# Label Encoding:
X['ever_married'] = np.where(X['ever_married'] == 'Yes', 1, 0)   # If married, replace with 1 otherwise 0
X['Residence_type'] = np.where(X['Residence_type'] == 'Rural', 1, 0)    # If residence type is Rural, replace with 1 otherwise 0
